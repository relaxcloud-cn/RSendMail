...
- **CLI + Tauri GUI 雙入口**：命令列和桌面圖形介面共用同一套 Rust 核心邏輯
- 批量處理和發送多封郵件
- 多執行緒處理提升效能
- 支援自訂 SMTP 伺服器設定
...
RSendMail 的 CLI 和 Tauri GUI 介面均支援多語言：
...
### Windows 使用
從 [Releases](https://github.com/kpassy/RSendMail/releases) 頁面下載 Windows CLI 執行檔（`rsendmail-cli-windows-x86_64.exe`）。
```bash
rsendmail-cli-windows-x86_64.exe --smtp-server <smtp伺服器> --port <連接埠> --from <寄件人> --to <收件人> --dir <郵件目錄> --processes <處理程序數> --batch-size <批次大小>
```
